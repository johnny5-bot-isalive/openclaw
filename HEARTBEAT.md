...
Use the project registry plus the global and project-local Kanban boards in The Nexus as the execution spine.
...
4. Otherwise, do not change global board state during heartbeat.
5. Before sending a progress update, re-read enough of the changed files to verify they match the new state.
6. If the registry or Kanban path cannot be read, send a concise blocker update naming the failing path.
...
3. For each eligible project, send a short wake to the recorded session or thread with `sessions_send`, telling it to read the local `Project Kanban.md`, take one real step from `Doing` or `Ready` if anything actionable exists, reconcile local board state, and report concise progress or a real blocker.
...
6. Treat a project as effectively clear when its local `Project Kanban.md` has no actionable `Doing`, `Ready`, or meaningful `Blocked` item. Do not keep re-waking a clear project just because an old cadence still says `every-heartbeat`.
